...
//for sensitivity testing
	  ...
	  if(L2AKuDataX.CSF.binBBPeak[j]-1>0)                 //WSO 05/01/2014 
	    {
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2-2; //WSO 05/01/14
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2+1;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2+3; //WSO 05/01/2014
	      inodes++;
	    }
	  else
	    {
...
//begin  WSO 9/13/13 changed > to >=
	  ...
	  if(L2AKuDataX.CSF.binBBPeak[j]>10)
	    raintype[ipia]=100;
//here, lower freezing level by 250 m for sensitivity test
