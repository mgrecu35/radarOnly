...
//for sensitivity testing
	  ...
	  if(L2AKuDataX.CSF.binBBPeak[j]-1>0)                 //WSO 05/01/2014 
	    {
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2-4; //WSO 05/01/14
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2-1;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2+1; //WSO 05/01/2014
	      inodes++;
	    }
...
//here, raise freezing level by 250 m for sensitivity test
	  freezH[ipia]=L2AKuDataX.VER.heightZeroDeg[j] + 250.;
//end here
