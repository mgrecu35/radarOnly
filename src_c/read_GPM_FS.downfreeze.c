...
//aa  begin  SFM  09/04/2013
    status = TKopen(&fname[0], "2AKuX", TKREAD, "HDF5", jobname,
		    &granuleHandle2AKu, 1);
...
//
//
//here add one 250 m gate to freezing level and transition boundaries
//for sensitivity testing
	  ...
	  if(L2AKuDataX.CSF.binBBPeak[j]-1>0)                 //WSO 05/01/2014 
	    {
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2-1; //WSO 05/01/14
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2+2;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.CSF.binBBPeak[j]-1)/2+4; //WSO 05/01/2014
	      inodes++;
	    }
	  else
	    {
	      nodes[inodes]=(L2AKuDataX.VER.binZeroDeg[j]-1)/2-1; //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.VER.binZeroDeg[j]-1)/2+2;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuDataX.VER.binZeroDeg[j]-1)/2+4; //WSO 05/01/2014
	      inodes++;
	    }
//end here
//
...
//begin  WSO 9/13/13 changed > to >=
	  ...
	  if(L2AKuDataX.CSF.binBBPeak[j]>10)
	    raintype[ipia]=100;
//here, raise freezing level by 250 m for sensitivity test
	  freezH[ipia]=L2AKuDataX.VER.heightZeroDeg[j] - 500.;
//end here
...
//		  &granuleHandle2AKu, 1);
  ...
  status_alpha = TKopen(&fname[0], "2AKuX", TKREAD, "HDF5", jobname,
		  &granuleHandle2AKu, 1);
  if (status_alpha != 0)
      {
