...
//
//
//here, for freezing level and top and bottom of mixed-phase layer,
//add one 250 m gate, for sensitivity tests
	  nodes[inodes]= (L2AKuData.PRE.binStormTop[j]-1)/2; //WSO 05/01/2014
	  inodes++;

	  if(L2AKuData.CSF.binBBPeak[j]-1>0)                 //WSO 05/01/2014 
	    {
	      nodes[inodes]=(L2AKuData.CSF.binBBPeak[j]-1)/2-1; //WSO 05/01/14
	      inodes++;
	      nodes[inodes]=(L2AKuData.CSF.binBBPeak[j]-1)/2+2;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuData.CSF.binBBPeak[j]-1)/2+4; //WSO 05/01/2014
	      inodes++;
	    }
	  else
	    {
	      nodes[inodes]=(L2AKuData.VER.binZeroDeg[j]-1)/2-1; //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuData.VER.binZeroDeg[j]-1)/2+2;   //WSO 05/01/2014
	      inodes++;
	      nodes[inodes]=(L2AKuData.VER.binZeroDeg[j]-1)/2+4; //WSO 05/01/2014
	      inodes++;
	    }
//end here
//
...
//here, raise freezing level by 250 m for sensitivity test
	  freezH[ipia]=L2AKuData.VER.heightZeroDeg[j] - 500.;
//end here
