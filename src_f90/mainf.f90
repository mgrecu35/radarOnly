...
!...Check file availability for processing
    ...
    CALL file_check(igmi1, igmi2, igmi3, i2aku, i2adpr, i2akuenv, isnow, iseaice,  &
		    ...
		    st_seaice, st_cmb, date_number, orbitNumber, ifs)
...
!begin WSO 8/7/20 write empty granule in FS mode
         if(ifs == 1) then
            CALL write_empty_fs()
         else
            CALL write_empty()
...
!end WSO 8/7/20
      ...
      print *, 'Calling meta_mini()'
      CALL meta_mini(algorithmVersion)
      print *, 'Calling closeoutputfilet()'
      CALL closeoutputfilet()
      print *, 'Back from closeoutputfilet()'
      ndpr1=-300
      RETURN
  ENDIF
!  SFM  end    01/02/2014
